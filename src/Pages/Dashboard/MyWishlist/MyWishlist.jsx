...
const MyWishlist = () => {
  ...
  const handleAddToCart = (wishlist) => {
    // Log the item name to the console
    console.log("Product Name:", wishlist.item_name);

    const isInCart = cartData?.some(
      (cartItem) => cartItem.item_name === wishlist.item_name
    );
  ...
  return (
    <>
      <Helmet>
        <title>Sip Coffee | My Wishlist</title>
      </Helmet>
      <div className="m-12">
        {wishlistData?.length > 0 ? (
          ...
          <div className="flex justify-center items-center mt-8">
            <p className="text-xl font-semibold">No wishlist found</p>
          </div>
        )}
      </div>
    </>
